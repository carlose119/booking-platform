...
- [ ] 3.1 Create `tests/Unit/Policies/EmployeeSchedulePolicyTest.php` for BusinessAdmin tenant CRUD and Employee own-schedule read-only access.
- [ ] 3.2 Create `tests/Feature/Filament/EmployeeScheduleResourceTest.php` for tenant-only listings/options, cross-tenant denial, valid hours, invalid day/time, and Employee write denial.
- [ ] 3.3 Run `php artisan test tests/Unit/Policies/EmployeeSchedulePolicyTest.php tests/Feature/Filament/EmployeeScheduleResourceTest.php` and confirm failures cover schedule rules.

## Phase 4: GREEN/REFACTOR — Employee Schedule Management

- [ ] 4.1 Create `app/Policies/EmployeeSchedulePolicy.php` with BusinessAdmin same-tenant CRUD and Employee own-schedule `viewAny/view` only.
- [ ] 4.2 Update `app/Filament/Resources/EmployeeScheduleResource.php` to scope query by role/tenant, filter employee options, validate `day_of_week` 0-6, and require `end_time` after `start_time`.
- [ ] 4.3 Hide or deny schedule create/edit/delete actions for Employees in `app/Filament/Resources/EmployeeScheduleResource.php`.
- [ ] 4.4 Register `EmployeeScheduleResource` in `app/Providers/Filament/TenantPanelProvider.php`; run schedule tests and `vendor/bin/pint --dirty`.

## Phase 5: Final Verification

- [ ] 5.1 Run `php artisan test tests/Unit/Policies tests/Feature/Filament/UserResourceTest.php tests/Feature/Filament/EmployeeScheduleResourceTest.php`.
- [ ] 5.2 Run `composer test` and confirm user-management, schedule-management, and rbac-roles scenarios pass.
