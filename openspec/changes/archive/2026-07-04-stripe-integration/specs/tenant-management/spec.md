...
The system SHALL store tenant records with at minimum a `name` and `slug` field, plus payment configuration fields: payment_policy (enum: 100upfront, fraction, nopayment), deposit_percentage (integer, nullable), refund_window_hours (integer), stripe_api_key (encrypted, nullable), stripe_webhook_secret (encrypted, nullable).
...
- THEN it has `id`, `name`, `slug`, `payment_policy`, `deposit_percentage`, `refund_window_hours`, `stripe_api_key`, `stripe_webhook_secret`, `created_at`, and `updated_at` columns
...
The system SHALL provide a Filament resource in the Super Admin panel for creating, reading, updating, and deleting tenant records. The resource MUST include payment configuration fields in the form.
...
- WHEN they submit the tenant creation form with valid data (name, slug, payment_policy)
...
- THEN all tenant fields including payment settings are displayed correctly
